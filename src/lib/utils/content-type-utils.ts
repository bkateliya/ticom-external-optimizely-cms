...
export type ContentPropsWithId<T extends ContentTypes.AnyContentType> =
  ContentProps<T> & { _id: string };
...
>(
  array: (GenericContentType | null | undefined)[] | undefined | null,
  onlyOfType?: T,
): (ContentProps<T> & { _id: string })[] {
  const typedArray =
    array
      ?.map((item) => normalizeGenericContentToTyped(item, onlyOfType))
      .filter((x) => x != null) ?? [];
  return typedArray;
}
