...
// Both icons come straight from TI's DAM, as they do in AEM. The domain isn't in
// next.config's remotePatterns, so they go through a plain <img>.
const ICON_BASE = `${SHARED_ENV_VARS.NEXT_PUBLIC_TICOM_BASE_DOMAIN}/content/dam/ticom/images/icons/illustrative-icons`;

// The reference design tool is still the legacy app, and it has no locale
// segment: /en-us/reference-designs 404s, AEM points translated pages at the old
// two-letter path (/jp/...). So this one link is left un-normalized.
...
// `!` on the type and margin classes: TI's global stylesheet restyles bare `h*`
// and `p` unlayered, which beats our layered utilities on the VM.
const style = tv({
  slots: {
