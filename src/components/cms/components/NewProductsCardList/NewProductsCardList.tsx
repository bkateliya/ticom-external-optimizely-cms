...
import {
  FeaturedProductsPartNumberInformation,
  getFeaturedProducts,
...
function getNewProductsUrl(locale: string) {
  ...
  if (locale === "en-us")
    ...
    return `${domain}/${encodeURIComponent(locale)}/product-category/new-products.html?releasePeriod=364`;
}
...
async function NewProductsCard({
  product,
  locale,
  hasFamily,
  ...
  const altText = product.partImageAvailable
    ? "TODO: fetch alt text from DAM"
    : "TI chip";

  const familyLink = (
    <div className={tv.familyContainer()}>
      <a
        href={product.selectionToolUrl}
        className={tv.family()}
  ...
  const newBadge = product.newFlag ? (
    <a
      className={tv.newBadge()}
      href={newProductsUrl}
      target="_blank"
      data-lid="newproducts-view-products"
    >
      {t("New product")}
    </a>
  ) : (
  ...
  return (
    <div className={tv.cardContainer()}>
      <TiCard
        className={tv.card()}
        appearance={hasFamily ? "plain-white" : "outlined-white"}
        dataLid="newproducts-view-products"
      >
        <a
          href={product.gpnUrl}
          slot="card-link"
          data-navtitle={product.genericPartNumber}
        >
          {product.genericPartNumber}
        </a>
        <div className={tv.content()}>{!hasFamily ? familyLink : <></>}</div>
        ...
        <div className={tv.details()}>
          ...
          <p className={tv.description()}>{product.deviceDescription}</p>
          {product.approximatePrice ? (
            <p className={tv.price()}>
              {t("Approx. price")} (
              <span className="js-currency">{product.currency}</span>)
              <span className={tv.priceVal()}>
                {product.displayQuantity} |
                <span
                  orig-price={product.approximatePrice}
                  className="js-price"
                >
                  {product.approximatePrice}
                </span>
...
export async function NewProductsCardListComponent({}: OptiComponentProps<
  typeof NewProductsCardListComponentType
>) {
  ...
  const featuredProductResult = await getFeaturedProducts({
    language: locale,
    familyId: hasFamily ? +familyId : null,
  });

  if (!featuredProductResult) {
    return <></>;
  }
  const { featuredProductInfo } = featuredProductResult;
  if (
    !featuredProductInfo.partNumberInformation ||
    featuredProductInfo.partNumberInformation.length === 0
  ) {
    return <></>;
  }
  const featuredProducts = await Promise.all(
    featuredProductInfo.partNumberInformation.map((product, i) => (
      <NewProductsCard
        key={`featuredproduct-${i}`}
        product={product}
        hasFamily={hasFamily}
        locale={locale}
      />
    ...
    <div>
      ...
      <div className={tv.carouselWrapper()}>
        <TiCarousel
          ...
          gap="large"
          navigation="below"
          slides={featuredProducts.map((featuredProduct) => ({
...
const TAILWIND_VARIANTS = tv({
  slots: {
    headingRow: ["flex", "justify-between", "items-center", "mb-4"],
    heading: ["mb-0"],
    ctaIcon: ["mr-1"],
    ctaLink: ["text-sm", "text-(--pl-link-color-primary)"],
    carouselWrapper: [],
    carousel: [],
    cardContainer: [
      "flex",
    ...
    status: ["flex", "justify-start", "items-center", "gap-2", "mb-3"],
    details: [],
    description: ["text-md", "mb-8", "text-(--pl-link-color-secondary)"],
    price: ["font-normal", "text-sm"],
    priceVal: [],
    newBadge: [
      "inline-block",
      "uppercase",
    ...
    name: ["mb-3", "font-normal", "text-2xl"],
    nameLink: ["text-(--pl-link-color-secondary)"],
  },
});
