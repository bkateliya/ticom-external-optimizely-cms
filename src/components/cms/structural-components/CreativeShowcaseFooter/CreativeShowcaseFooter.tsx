...
import { CreativeShowcaseFooterComponentType } from "./CreativeShowcaseFooter.model";
...
export async function CreativeShowcaseFooter({
  ...
  const { WrappedRichTextField } =
    fieldFactory<typeof CreativeShowcaseFooterComponentType>(content);

  const footerLinks = normalizeGenericArrayToTyped<typeof LinkElementType>(
    content.footerLinks,
  );

  const copyrightText = t("CopyrightText-{year}", {
    year: String(new Date().getFullYear()),
  });

  return (
    <footer id="tiResponsiveFooter" className="ti_p-responsiveFooter">
      <div className="ti_p-responsiveFooter-content">
        <div className="ti_p-responsiveFooter-row">
          <div className="ti_p-responsiveFooter-col mod-half">
            <NextLink
              href={TI_LOGO_HREF}
              className="ti_p-responsiveFooter-logo"
              data-navtitle="footer_logo_link"
            >
              <div className="ti_p-responsiveFooter-logo-container">
                <TiWordmark />
              </div>
            </NextLink>
          </div>

          {content.importantInformation?.json && (
            <div className="ti_p-responsiveFooter-col mod-half">
              {/* A div, not a p: the rich text renders its own block elements. */}
              <div className="ti_p-responsiveFooter-copy">
                <WrappedRichTextField field="importantInformation" />
              </div>
            </div>
          )}
        </div>
        ...
        {footerLinks?.length > 0 && (
          <div className="ti_p-responsiveFooter-row">
            <div className="ti_p-responsiveFooter-col mod-full">
              <div className="ti_p-responsiveFooter-inlineLinks">
                <div className="ti_p-responsiveFooter-links-overflow">
                  <div className="ti_p-responsiveFooter-links-list mod-inline">
                    {footerLinks.map((link, index) => (
                      <ul
                        key={index}
                        className="ti_p-responsiveFooter-links-list mod-inline"
                      >
                        <li className="mb-2">
                          <FooterLink
                            content={link}
                            parentField={`footerLinks.${index}`}
                          />
                        </li>
                      </ul>
                    ))}
        ...
        <div className="ti_p-responsiveFooter-row">
          <div className="ti_p-responsiveFooter-col mod-full">
            <p className="ti_p-responsiveFooter-copyright">
              <CopyrightLink content={content} label={copyrightText} />{" "}
              {t("CopyrightSuffixText")}
            </p>
          </div>
        </div>
      </div>
...
function CopyrightLink({
  ...
  return (
    <NextLink
      {...pa("copyrightLinkText")}
      data-navtitle="footer_copyright"
      href={url}
...
/** One entry of the inline link list. */
function FooterLink({
  content,
  parentField,
}: OptiComponentProps<typeof LinkElementType>) {
  const link = content?.link;
  /* `url.default` is the relative path with the locale already in it; prepending
       `url.base` would bake the current host into the href. */
  const href = link?.url?.default;
  ...
  return (
    <NextLink
      ...
      className="ti_p-responsiveFooter-link mod-bordered"
      data-navtitle={link.text}
      href={url}
      target={link.target ?? undefined}
    >
      {link.text}
